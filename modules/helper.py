...
def moveEnemy(enemy, data, currentRoomData, Player, currentTime, isHit=False, slowdown=False, deltaTime=0):
	...
	distance_fromOffsetPoint = measureDistance((Player.hitbox.center[0]+enemy.customAttributes["random steer"][0],Player.hitbox.center[1]+enemy.customAttributes["random steer"][1]), enemy.hitbox.center)
	if (not slowdown):
	...
	if (data["FLAGS"][2] in enemy.customAttributes["flags"]):
		...
		if (enemy.customAttributes["state"] == 1):
			if (distance_fromOffsetPoint < 500 and distance_fromOffsetPoint > enemy.customAttributes["pursue distance"]):
				enemy.customAttributes["state"] = PURSUING
			elif (distance_fromOffsetPoint <= enemy.customAttributes["pursue distance"]):
		...
		if (enemy.customAttributes["state"] == ATTACK):
			if (timerFinishCheck(currentTime, enemy.customAttributes["state timer start"], enemy.customAttributes["attack duration"]*durationMultiplier)):
				...
				enemy.customAttributes["random steer"] = (random.randint(-enemy.customAttributes["steer offset"],enemy.customAttributes["steer offset"]),random.randint(-enemy.customAttributes["steer offset"],enemy.customAttributes["steer offset"]))
		...
		if (enemy.customAttributes["state"] == PURSUING):
			movement_vector = goto_angleComplex(enemy, speed_multiplier=1, angle=face_target(enemy.hitbox.center, (Player.hitbox.center[0]+enemy.customAttributes["random steer"][0],Player.hitbox.center[1]+enemy.customAttributes["random steer"][1])), targetPos=Player.hitbox.center, checkCollision=True, collisionList=checkCollisionList, setDir = True) 
